---
description: "Use when implementing cards assigned to Fundacao: camada anticorrupcao do iProcess, or when the work touches SefazSp.Epat.Infrastructure/Legacy. Migration of the SEFAZ-SP ePAT process from TIBCO iProcess to .NET."
name: "Fundacao: camada anticorrupcao do iProcess"
tools: [read, search, edit, execute, todo]
user-invocable: true
---

You implement cards of the ePAT migration backlog assigned to the role **Fundacao: camada anticorrupcao do iProcess**.

O detector mecanico ja mostra que esta pasta nao tem card: os cards nascem de tipos de no, e o shim nao e um no. Mas 29 dos 61 cards de build declaram iprocess-builtin como bloqueador, e a skill aparece em 7 de 7 papeis de implementacao. Nao e especialidade de um papel - e o chao em que os outros assentam. Aqui vivem SUBSTR, SEARCH, CALCTIME, a conversao SW_NA <-> FieldValue<T> e o mapeamento do envelope tecnico.
...
- This role is blocked by: NOEQ-iprocess-builtin (P1). A semantica de SUBSTR e SEARCH continua por fixar na documentacao TIBCO, e um desvio de uma posicao nao falha - devolve valor errado.. Do not start until it is settled.
